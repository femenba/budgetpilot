...
export default function AddExpense() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    const { error } = await createExpense(user.id, {
      amount:       num,
    ...
    <Layout>
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold text-ink tracking-tight">Add Expense</h1>
            <p className="text-sm text-dim">Record a new expense entry</p>
          </div>
        </div>
        ...
        {saved ? (
          <div className="flex flex-col items-center py-16 text-center">
            <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center mb-4">
              <CheckCircle2 size={28} className="text-accent-red" />
            </div>
            <h2 className="text-base font-semibold text-ink">Expense saved!</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="bg-surface rounded-2xl border border-line shadow-card p-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  placeholder="0.00"
                  required
                  className="flex-1 text-4xl font-bold text-accent-red bg-transparent border-none outline-none placeholder:text-line w-full"
                  autoFocus
                />
              </div>
              <div className="mt-3 h-px bg-gradient-to-r from-red-200 via-red-100 to-transparent" />
            </div>
            ...
            <div className="bg-surface rounded-2xl border border-line shadow-card p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between py-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-medium text-ink">Recurring expense</p>
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${recurring ? 'bg-accent-red' : 'bg-line'}`}
                >
            ...
            {amount && parseFloat(amount) > 0 && (
              <div className="flex items-center justify-between px-4 py-3 bg-red-50 rounded-xl border border-red-100">
                <div className="text-sm text-red-700">
                  ...
                  {description && <span className="text-accent-red ml-1">· {description}</span>}
                </div>
            ...
            <div className="flex gap-3 pb-4">
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex-1 py-3.5 rounded-xl bg-accent-red hover:bg-red-600 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-semibold transition-all shadow-sm"
              >
                {saving ? (
